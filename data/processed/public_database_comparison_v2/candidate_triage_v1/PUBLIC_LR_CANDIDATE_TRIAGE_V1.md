...
| Completed batch review | 6,000 | Resolved in a batch-specific evidence ledger; excluded from new review batches while retaining the resolution record. |
...
| Discovery review | 760 | No automatic graph match and no primary locator in the frozen public snapshot. |

The actionable review queue contains `760` units, of which `0` have a primary locator and `760` require discovery or broader evidence search.
