...
| Completed batch review | 6,200 | Resolved in a batch-specific evidence ledger; excluded from new review batches while retaining the resolution record. |
...
| Discovery review | 560 | No automatic graph match and no primary locator in the frozen public snapshot. |

The actionable review queue contains `560` units, of which `0` have a primary locator and `560` require discovery or broader evidence search.
