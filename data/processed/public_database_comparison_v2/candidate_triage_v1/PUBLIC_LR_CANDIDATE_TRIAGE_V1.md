...
This report reduces `7,753` public-only rows to `7,753` normalized candidate units against `8,112` direct ligand--receptor graph edges from `module20_24_mechanism_graph:2026-09-21`.
...
| Completed batch review | 1,900 | Resolved in a batch-specific evidence ledger; excluded from new review batches while retaining the resolution record. |
...
| Primary-evidence review | 484 | No automatic graph match and a public primary-paper locator is present. |
| Discovery review | 4,376 | No automatic graph match and no primary locator in the frozen public snapshot. |

The actionable review queue contains `4,860` units, of which `484` have a primary locator and `4,376` require discovery or broader evidence search.
