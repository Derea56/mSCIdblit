...
def test_sci_protein_context_curation_overrides_are_applied_with_source_provenance():
    ...
    assert {row["sex"] for row in by_study["FLOW_SCI_061"]} == {"adult male Wistar rats, 250-300 g"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_231"]} == {"T9"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_231"]} == {"75 kilodynes; severe contusion"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_325"]} == {"T10"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_325"]} == {"60 kilodyne moderate contusion"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_026"]} == {"T10"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_026"]} == {"150 kdynes with 1-second dwell"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_075"]} == {"T10"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_075"]} == {"Severe crush; constant No. 5 Dumont forceps compression for 5 seconds"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_223"]} == {"T10/T11"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_223"]} == {"Mild: 5 g × 10 mm; severe: 5 g × 40 mm"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_185"]} == {"T9"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_185"]} == {"50 kDynes moderate contusion"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_381"]} == {"T11"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_381"]} == {"50 kDynes moderate contusion; 500-600 micrometre tissue displacement"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_380"]} == {"T9/10"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_380"]} == {"Moderate contusion; 0.8-mm spinal-cord displacement"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_443"]} == {"T8-T9"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_443"]} == {"0.4-mm lateral forceps compression for 15 seconds"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_384"]} == {"T10"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_384"]} == {"5-g weight dropped from 11 mm"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_021"]} == {"T10"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_021"]} == {"Mild: 12.5 mm; moderate: 25.0 mm; severe: 50.0 mm MASCIS drop heights"}
    assert {row["sex"] for row in by_study["FLOW_SCI_021"]} == {"adult female Sprague-Dawley rats, 230-250 g"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_011"]} == {"T8"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_011"]} == {"moderate-to-severe contusion; 10-g weight dropped 25 mm"}
    assert {row["sample_count"] for row in by_study["FLOW_SCI_011"]} == {"5"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_035"]} == {"70 kDyn contusion; 600-800 micrometre displacement"}
    assert {row["sex"] for row in by_study["FLOW_SCI_035"]} == {"unknown"}
    assert {row["sample_scope"] for row in by_study["FLOW_SCI_035"]} == {"approximately 4-mm lesion-centered spinal-cord segment"}
    ...
    assert {row["injury_level"] for row in by_study["FLOW_SCI_044"]} == {"T9/T10"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_044"]} == {"complete hind-limb paralysis reported; exact clip force not extracted"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_050"]} == {"T9/T10 segment exposed through T7/T8 laminectomy"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_050"]} == {"moderate contusion"}
    assert {row["sex"] for row in by_study["FLOW_SCI_083"]} == {"female"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_102"]} == {"T10"}
    assert {row["sex"] for row in by_study["FLOW_SCI_102"]} == {"female"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_123"]} == {"forceps fully clamped for 2 seconds"}
    assert {row["sex"] for row in by_study["FLOW_SCI_123"]} == {"female"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_250"]} == {"T12"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_250"]} == {"60-kDyn hemitransection/contusion"}
    assert {row["sex"] for row in by_study["FLOW_SCI_250"]} == {"male C57BL/6J mice, 12 weeks"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_264"]} == {"T9"}
    assert {row["sex"] for row in by_study["FLOW_SCI_264"]} == {"male and female"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_321"]} == {"T9-T10"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_391"]} == {"T8"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_401"]} == {"T8"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_140"]} == {"moderate; 50 kdyn contusion in the primary SCI experiments"}
    assert {row["sex"] for row in by_study["FLOW_SCI_360"]} == {"female"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_351"]} == {"T9"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_347"]} == {"T9 thoracic contusion; lumbar tissue analyzed for remote response"}
    ...
    assert {row["injury_level"] for row in by_study["FLOW_SCI_006"]} == {"T11"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_006"]} == {"60 kdyn contusion; 500-700 micrometre displacement"}
    assert {row["sex"] for row in by_study["FLOW_SCI_006"]} == {"adult female C57BL/6 mice, 8-10 weeks"}
    assert {row["injury_level"] for row in by_study["FLOW_SCI_019"]} == {"T10"}
    assert {row["injury_severity"] for row in by_study["FLOW_SCI_019"]} == {"0.4-mm forceps-tip bilateral compression for 2 seconds; complete crush"}
    assert {row["sex"] for row in by_study["FLOW_SCI_019"]} == {"male C57BL/6J mice, 8-10 weeks, 19-21 g"}
    assert {row["sample_scope"] for row in by_study["FLOW_SCI_019"]} == {"5-mm injury-centered spinal-cord segment for western blot"}
