...
def test_one_hundred_thirty_sixth_primary_literature_expansion_batch_preserves_p1_receptor_proximal_adaptor_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch136.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion135"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion136"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1085, 1095)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:00512",
        "M21B-DOWNSTREAM:00519",
        "M21B-DOWNSTREAM:00520",
        "M21B-DOWNSTREAM:00521",
        "M21B-DOWNSTREAM:00522",
        "M21B-DOWNSTREAM:00523",
        "M21B-DOWNSTREAM:00540",
        "M21B-DOWNSTREAM:00541",
        "M21B-DOWNSTREAM:00549",
        "M21B-DOWNSTREAM:00550",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_intracellular_output_missing_tf",
    }
    ...
    assert {row["source_evidence_record_id"] for row in rows} == {
        f"M21B-DOWNSTREAM-EVID:{number:06d}" for number in (900, 913, 917, 921, 925, 928, 947, 948, 963, 964)
    }
    assert all(row["intracellular_status"] == "source_supported" for row in rows)
    assert all(row["intracellular_continuation_node_id"] for row in rows)
    assert all(not row["transcription_factor_node_id"] for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1064
    assert len(all_queue_ids - covered_after) == 2267
    assert len(p1_queue_ids - covered_after) == 1235

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17141
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1094
    assert {row["source_chain_id"] for row in route_rows[-10:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_thirty_seventh_primary_literature_expansion_batch_preserves_p1_checkpoint_and_adaptor_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch137.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion136"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion137"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1095, 1105)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:00470",
        "M21B-DOWNSTREAM:00471",
        "M21B-DOWNSTREAM:00516",
        "M21B-DOWNSTREAM:00517",
        "M21B-DOWNSTREAM:00518",
        "M21B-DOWNSTREAM:00524",
        "M21B-DOWNSTREAM:00525",
        "M21B-DOWNSTREAM:00829",
        "M21B-DOWNSTREAM:00846",
        "M21B-DOWNSTREAM:00857",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_intracellular_output_missing_tf",
    }
    ...
    assert {row["source_evidence_record_id"] for row in rows} == {
        "M21B-DOWNSTREAM-EVID:000826",
        "M21B-DOWNSTREAM-EVID:000827",
        "M21B-DOWNSTREAM-EVID:000907",
        "M21B-DOWNSTREAM-EVID:000909",
        "M21B-DOWNSTREAM-EVID:000911",
        "M21B-DOWNSTREAM-EVID:000929",
        "M21B-DOWNSTREAM-EVID:000930",
        "M21B-DOWNSTREAM-EVID:001367",
        "M21B-DOWNSTREAM-EVID:001402",
        "M21B-DOWNSTREAM-EVID:001416",
    }
    assert all(row["intracellular_status"] == "source_supported" for row in rows)
    assert all(row["intracellular_continuation_node_id"] for row in rows)
    assert all(not row["transcription_factor_node_id"] for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1074
    assert len(all_queue_ids - covered_after) == 2257
    assert len(p1_queue_ids - covered_after) == 1225

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17151
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1104
    assert {row["source_chain_id"] for row in route_rows[-10:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_thirty_eighth_primary_literature_expansion_batch_preserves_p1_myeloid_and_nk_adaptor_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch138.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion137"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion138"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1105, 1115)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:00781",
        "M21B-DOWNSTREAM:00833",
        "M21B-DOWNSTREAM:00837",
        "M21B-DOWNSTREAM:00838",
        "M21B-DOWNSTREAM:00850",
        "M21B-DOWNSTREAM:00851",
        "M21B-DOWNSTREAM:00854",
        "M21B-DOWNSTREAM:00855",
        "M21B-DOWNSTREAM:00858",
        "M21B-DOWNSTREAM:00859",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_intracellular_output_missing_tf",
    }
    ...
    assert {row["source_evidence_record_id"] for row in rows} == {
        "M21B-DOWNSTREAM-EVID:001279",
        "M21B-DOWNSTREAM-EVID:001375",
        "M21B-DOWNSTREAM-EVID:001383",
        "M21B-DOWNSTREAM-EVID:001384",
        "M21B-DOWNSTREAM-EVID:001408",
        "M21B-DOWNSTREAM-EVID:001409",
        "M21B-DOWNSTREAM-EVID:001412",
        "M21B-DOWNSTREAM-EVID:001413",
        "M21B-DOWNSTREAM-EVID:001417",
        "M21B-DOWNSTREAM-EVID:001419",
    }
    assert all(row["intracellular_status"] == "source_supported" for row in rows)
    assert all(row["intracellular_continuation_node_id"] for row in rows)
    assert all(not row["transcription_factor_node_id"] for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1084
    assert len(all_queue_ids - covered_after) == 2247
    assert len(p1_queue_ids - covered_after) == 1215

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17161
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1114
    assert {row["source_chain_id"] for row in route_rows[-10:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_thirty_ninth_primary_literature_expansion_batch_preserves_p1_receptor_output_and_ret_signal_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch139.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion138"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion139"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1115, 1125)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:00551",
        "M21B-DOWNSTREAM:02200",
        "M21B-DOWNSTREAM:02201",
        "M21B-DOWNSTREAM:02204",
        "M21B-DOWNSTREAM:02205",
        "M21B-DOWNSTREAM:02206",
        "M21B-DOWNSTREAM:02207",
        "M21B-DOWNSTREAM:02238",
        "M21B-DOWNSTREAM:02239",
        "M21B-DOWNSTREAM:02240",
    }
    assert {
        row["route_tier"] for row in rows
    } == {
    ...
    assert sum(row["route_tier"] == "ligand_receptor_intracellular_output_missing_tf" for row in rows) == 4
    assert sum(row["route_tier"] == "ligand_receptor_output_annotation_missing_intracellular_and_tf" for row in rows) == 6
    assert {row["output_class"] for row in rows} == {
        "cellular_functional_readout",
        "generic_functional_readout",
        "phagocytosis_or_engulfment",
    ...
    assert {row["source_evidence_record_id"] for row in rows} == {
        "M21B-DOWNSTREAM-EVID:000965",
        "M21B-DOWNSTREAM-EVID:003516",
        "M21B-DOWNSTREAM-EVID:003518",
        "M21B-DOWNSTREAM-EVID:003528",
        "M21B-DOWNSTREAM-EVID:003530",
        "M21B-DOWNSTREAM-EVID:003532",
        "M21B-DOWNSTREAM-EVID:003535",
        "M21B-DOWNSTREAM-EVID:003587",
        "M21B-DOWNSTREAM-EVID:003588",
        "M21B-DOWNSTREAM-EVID:003589",
    }
    assert sum(row["intracellular_status"] == "source_supported" for row in rows) == 4
    assert sum(row["intracellular_status"] == "not_assayed" for row in rows) == 6
    assert all(not row["transcription_factor_node_id"] for row in rows)
    assert all(not row["target_gene_node_id"] for row in rows)
    assert all(row["causal_status"] == "not_asserted" for row in rows)
    assert all(row["traversal_status"] == "evidence_route_not_causal" for row in rows)
    ...
    for row in rows:
        ...
        if row["receptor_intracellular_edge_id"]:
            assert edges[row["receptor_intracellular_edge_id"]][1] == row["intracellular_continuation_node_id"]
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1094
    assert len(all_queue_ids - covered_after) == 2237
    assert len(p1_queue_ids - covered_after) == 1205

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17171
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1124
    assert {row["source_chain_id"] for row in route_rows[-10:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_fortieth_primary_literature_expansion_batch_preserves_p1_receptor_output_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch140.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion139"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion140"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1125, 1135)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:00819",
        "M21B-DOWNSTREAM:00828",
        "M21B-DOWNSTREAM:00902",
        "M21B-DOWNSTREAM:01296",
        "M21B-DOWNSTREAM:01542",
        "M21B-DOWNSTREAM:01651",
        "M21B-DOWNSTREAM:01652",
        "M21B-DOWNSTREAM:01817",
        "M21B-DOWNSTREAM:02015",
        "M21B-DOWNSTREAM:02022",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_output_annotation_missing_intracellular_and_tf",
    }
    ...
    assert sum(row["effect_polarity"] == "inhibitory" for row in rows) == 1
    assert all(row["intracellular_status"] == "not_assayed" for row in rows)
    assert all(not row["transcription_factor_node_id"] for row in rows)
    assert all(not row["target_gene_node_id"] for row in rows)
    assert all(row["causal_status"] == "not_asserted" for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1104
    assert len(all_queue_ids - covered_after) == 2227
    assert len(p1_queue_ids - covered_after) == 1195

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17181
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1134
    assert {row["source_chain_id"] for row in route_rows[-10:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_forty_first_primary_literature_expansion_batch_preserves_receptor_proximal_continuation_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch141.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion140"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion141"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1135, 1145)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:00587",
        "M21B-DOWNSTREAM:01116",
        "M21B-DOWNSTREAM:01144",
        "M21B-DOWNSTREAM:01150",
        "M21B-DOWNSTREAM:01151",
        "M21B-DOWNSTREAM:01191",
        "M21B-DOWNSTREAM:01706",
        "M21B-DOWNSTREAM:00889",
        "M21B-DOWNSTREAM:00890",
        "M21B-DOWNSTREAM:00891",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_intracellular_output_missing_tf",
    }
    ...
    assert sum(row["output_class"] == "gene_expression_or_transcription" for row in rows) == 1
    assert sum(row["output_class"] == "translocation_or_localization_readout" for row in rows) == 2
    assert sum(row["output_class"] == "cellular_functional_readout" for row in rows) == 4
    assert all(not row["transcription_factor_node_id"] for row in rows)
    assert all(not row["target_gene_node_id"] for row in rows)
    assert all(row["causal_status"] == "not_asserted" for row in rows)
    assert all(row["traversal_status"] == "evidence_route_not_causal" for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1114
    assert len(all_queue_ids - covered_after) == 2217
    assert len(p1_queue_ids - covered_after) == 1185

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17191
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1144
    assert {row["source_chain_id"] for row in route_rows[-10:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_forty_second_primary_literature_expansion_batch_preserves_receptor_proximal_continuation_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch142.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion141"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion142"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1145, 1155)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:00543",
        "M21B-DOWNSTREAM:00544",
        "M21B-DOWNSTREAM:01154",
        "M21B-DOWNSTREAM:01719",
        "M21B-DOWNSTREAM:01721",
        "M21B-DOWNSTREAM:01740",
        "M21B-DOWNSTREAM:02031",
        "M21B-DOWNSTREAM:01997",
        "M21B-DOWNSTREAM:01843",
        "M21B-DOWNSTREAM:01145",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_intracellular_output_missing_tf",
    }
    ...
    assert sum(row["output_class"] == "gene_expression_or_transcription" for row in rows) == 1
    assert sum(row["output_class"] == "phosphorylation_or_activation_readout" for row in rows) == 1
    assert sum(row["output_class"] == "reporter_readout" for row in rows) == 1
    assert sum(row["output_class"] == "generic_functional_readout" for row in rows) == 1
    assert all(not row["transcription_factor_node_id"] for row in rows)
    assert all(not row["target_gene_node_id"] for row in rows)
    assert all(row["causal_status"] == "not_asserted" for row in rows)
    assert all(row["traversal_status"] == "evidence_route_not_causal" for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1124
    assert len(all_queue_ids - covered_after) == 2207
    assert len(p1_queue_ids - covered_after) == 1175

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17201
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1154
    assert {row["source_chain_id"] for row in route_rows[-10:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_forty_third_primary_literature_expansion_batch_preserves_receptor_proximal_continuation_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch143.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion142"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion143"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1155, 1165)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:00914",
        "M21B-DOWNSTREAM:01073",
        "M21B-DOWNSTREAM:01194",
        "M21B-DOWNSTREAM:01584",
        "M21B-DOWNSTREAM:01615",
        "M21B-DOWNSTREAM:01718",
        "M21B-DOWNSTREAM:01720",
        "M21B-DOWNSTREAM:01910",
        "M21B-DOWNSTREAM:02148",
        "M21B-DOWNSTREAM:02192",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_intracellular_output_missing_tf",
    }
    assert all(row["intracellular_status"] == "source_supported" for row in rows)
    assert sum(row["output_class"] == "phosphorylation_or_activation_readout" for row in rows) == 2
    assert sum(row["output_class"] == "translocation_or_localization_readout" for row in rows) == 1
    assert sum(row["output_class"] == "gene_expression_or_transcription" for row in rows) == 1
    assert sum(row["output_class"] == "cellular_functional_readout" for row in rows) == 3
    assert sum(row["output_class"] == "phagocytosis_or_engulfment" for row in rows) == 1
    assert sum(row["output_class"] == "reporter_readout" for row in rows) == 1
    assert sum(row["output_class"] == "generic_functional_readout" for row in rows) == 1
    assert all(not row["transcription_factor_node_id"] for row in rows)
    assert all(not row["target_gene_node_id"] for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1134
    assert len(all_queue_ids - covered_after) == 2197
    assert len(p1_queue_ids - covered_after) == 1165

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17211
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1164
    assert {row["source_chain_id"] for row in route_rows[-10:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_forty_fourth_primary_literature_expansion_batch_preserves_receptor_proximal_continuation_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch144.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion143"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion144"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1165, 1175)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:00880",
        "M21B-DOWNSTREAM:00881",
        "M21B-DOWNSTREAM:00882",
        "M21B-DOWNSTREAM:00926",
        "M21B-DOWNSTREAM:00927",
        "M21B-DOWNSTREAM:01201",
        "M21B-DOWNSTREAM:01212",
        "M21B-DOWNSTREAM:01306",
        "M21B-DOWNSTREAM:01322",
        "M21B-DOWNSTREAM:01558",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_intracellular_output_missing_tf",
    }
    ...
    assert sum(row["output_class"] == "cleavage_or_processing_readout" for row in rows) == 1
    assert sum(row["output_class"] == "translocation_or_localization_readout" for row in rows) == 1
    assert sum(row["output_class"] == "phosphorylation_or_activation_readout" for row in rows) == 1
    assert sum(row["output_class"] == "reporter_readout" for row in rows) == 1
    assert all(not row["transcription_factor_node_id"] for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1144
    assert len(all_queue_ids - covered_after) == 2187
    assert len(p1_queue_ids - covered_after) == 1155
...
def test_one_hundred_forty_sixth_primary_literature_expansion_batch_preserves_receptor_proximal_continuation_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch146.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion145"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion146"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1185, 1195)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:01726",
        "M21B-DOWNSTREAM:01760",
        "M21B-DOWNSTREAM:01765",
        "M21B-DOWNSTREAM:01773",
        "M21B-DOWNSTREAM:01983",
        "M21B-DOWNSTREAM:02026",
        "M21B-DOWNSTREAM:02039",
        "M21B-DOWNSTREAM:02050",
        "M21B-DOWNSTREAM:02109",
        "M21B-DOWNSTREAM:02162",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_intracellular_output_missing_tf",
    }
    assert all(row["intracellular_status"] == "source_supported" for row in rows)
    assert sum(row["output_class"] == "cellular_functional_readout" for row in rows) == 7
    assert sum(row["output_class"] == "gene_expression_or_transcription" for row in rows) == 2
    assert sum(row["output_class"] == "secreted_protein_or_release" for row in rows) == 1
    assert all(not row["transcription_factor_node_id"] for row in rows)
    assert all(not row["target_gene_node_id"] for row in rows)
    assert all(row["causal_status"] == "not_asserted" for row in rows)
    assert all(row["traversal_status"] == "evidence_route_not_causal" for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1164
    assert len(all_queue_ids - covered_after) == 2167
    assert len(p1_queue_ids - covered_after) == 1135

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17241
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1194
    assert {row["source_chain_id"] for row in route_rows[-10:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_forty_seventh_primary_literature_expansion_batch_preserves_receptor_proximal_continuation_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch147.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion146"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion147"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1195, 1205)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:01687",
        "M21B-DOWNSTREAM:01715",
        "M21B-DOWNSTREAM:01736",
        "M21B-DOWNSTREAM:01981",
        "M21B-DOWNSTREAM:02046",
        "M21B-DOWNSTREAM:02047",
        "M21B-DOWNSTREAM:02107",
        "M21B-DOWNSTREAM:02186",
        "M21B-DOWNSTREAM:02187",
        "M21B-DOWNSTREAM:02188",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_intracellular_output_missing_tf",
    }
    assert all(row["intracellular_status"] == "source_supported" for row in rows)
    assert sum(row["output_class"] == "secreted_protein_or_release" for row in rows) == 2
    assert sum(row["output_class"] == "phagocytosis_or_engulfment" for row in rows) == 2
    assert sum(row["output_class"] == "cellular_functional_readout" for row in rows) == 2
    assert sum(row["output_class"] == "cleavage_or_processing_readout" for row in rows) == 1
    assert sum(row["output_class"] == "phosphorylation_or_activation_readout" for row in rows) == 3
    assert all(not row["transcription_factor_node_id"] for row in rows)
    assert all(not row["target_gene_node_id"] for row in rows)
    assert all(row["causal_status"] == "not_asserted" for row in rows)
    assert all(row["traversal_status"] == "evidence_route_not_causal" for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1174
    assert len(all_queue_ids - covered_after) == 2157
    assert len(p1_queue_ids - covered_after) == 1125

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17251
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1204
    assert {row["source_chain_id"] for row in route_rows[-10:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_forty_eighth_primary_literature_expansion_batch_preserves_receptor_proximal_continuation_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch148.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion147"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion148"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1205, 1215)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:01100",
        "M21B-DOWNSTREAM:01157",
        "M21B-DOWNSTREAM:01199",
        "M21B-DOWNSTREAM:01255",
        "M21B-DOWNSTREAM:01307",
        "M21B-DOWNSTREAM:01308",
        "M21B-DOWNSTREAM:01711",
        "M21B-DOWNSTREAM:02034",
        "M21B-DOWNSTREAM:02086",
        "M21B-DOWNSTREAM:02087",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_intracellular_output_missing_tf",
    }
    assert all(row["intracellular_status"] == "source_supported" for row in rows)
    assert sum(row["output_class"] == "cellular_functional_readout" for row in rows) == 3
    assert sum(row["output_class"] == "gene_expression_or_transcription" for row in rows) == 3
    assert sum(row["output_class"] == "phosphorylation_or_activation_readout" for row in rows) == 2
    assert sum(row["output_class"] == "translocation_or_localization_readout" for row in rows) == 1
    assert sum(row["output_class"] == "secreted_protein_or_release" for row in rows) == 1
    assert all(not row["transcription_factor_node_id"] for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1184
    assert len(all_queue_ids - covered_after) == 2147
    assert len(p1_queue_ids - covered_after) == 1115

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17261
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1214
    assert {row["source_chain_id"] for row in route_rows[-10:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_forty_ninth_primary_literature_expansion_batch_preserves_receptor_proximal_continuation_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch149.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion148"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion149"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 10
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1215, 1225)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:00928",
        "M21B-DOWNSTREAM:00929",
        "M21B-DOWNSTREAM:00930",
        "M21B-DOWNSTREAM:00931",
        "M21B-DOWNSTREAM:00934",
        "M21B-DOWNSTREAM:01109",
        "M21B-DOWNSTREAM:01110",
        "M21B-DOWNSTREAM:01111",
        "M21B-DOWNSTREAM:01205",
        "M21B-DOWNSTREAM:01206",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_intracellular_output_missing_tf",
    }
    assert all(row["intracellular_status"] == "source_supported" for row in rows)
    assert sum(row["output_class"] == "gene_expression_or_transcription" for row in rows) == 5
    assert sum(row["output_class"] == "translocation_or_localization_readout" for row in rows) == 2
    assert sum(row["output_class"] == "phosphorylation_or_activation_readout" for row in rows) == 2
    assert sum(row["output_class"] == "generic_functional_readout" for row in rows) == 1
    assert all(not row["transcription_factor_node_id"] for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1194
    assert len(all_queue_ids - covered_after) == 2137
    assert len(p1_queue_ids - covered_after) == 1105

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17271
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1224
    assert {row["source_chain_id"] for row in route_rows[-10:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_fiftieth_primary_literature_expansion_batch_preserves_receptor_proximal_continuation_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch150.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion149"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion150"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 11
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1225, 1236)
    }
    assert {row["source_queue_id"] for row in rows} == {
        "M21B-DOWNSTREAM:01143",
        "M21B-DOWNSTREAM:01153",
        "M21B-DOWNSTREAM:01210",
        "M21B-DOWNSTREAM:01319",
        "M21B-DOWNSTREAM:01685",
        "M21B-DOWNSTREAM:01908",
        "M21B-DOWNSTREAM:02045",
        "M21B-DOWNSTREAM:02088",
        "M21B-DOWNSTREAM:02089",
        "M21B-DOWNSTREAM:02151",
        "M21B-DOWNSTREAM:02236",
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_intracellular_output_missing_tf",
    }
    ...
    assert sum(row["output_class"] == "gene_expression_or_transcription" for row in rows) == 2
    assert sum(row["output_class"] == "phosphorylation_or_activation_readout" for row in rows) == 2
    assert sum(row["output_class"] == "translocation_or_localization_readout" for row in rows) == 5
    assert all(not row["transcription_factor_node_id"] for row in rows)
    assert all(not row["target_gene_node_id"] for row in rows)
    assert all(row["causal_status"] == "not_asserted" for row in rows)
    assert all(row["traversal_status"] == "evidence_route_not_causal" for row in rows)
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1205
    assert len(all_queue_ids - covered_after) == 2126
    assert len(p1_queue_ids - covered_after) == 1094

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17282
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1235
    assert {row["source_chain_id"] for row in route_rows[-11:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_fifty_first_primary_literature_expansion_batch_resolves_canonical_ligand_receptor_output_routes():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch151.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion150"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion151"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 594
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1236, 1830)
    }
    assert {row["route_tier"] for row in rows} == {
        "ligand_receptor_output_annotation_missing_intracellular_and_tf",
    }
    ...
    assert all(row["intracellular_status"] == "not_mapped" for row in rows)
    assert all(not row["receptor_intracellular_edge_id"] for row in rows)
    assert all(not row["intracellular_continuation_node_id"] for row in rows)
    assert all(not row["transcription_factor_node_id"] for row in rows)
    assert all(not row["target_gene_node_id"] for row in rows)
    assert all(not row["output_node_id"] for row in rows)
    assert all(row["causal_status"] == "not_asserted" for row in rows)
    assert all(row["traversal_status"] == "evidence_route_not_causal" for row in rows)
    assert all(row["curation_status"] == "curated_primary_route" for row in rows)
    assert {
        row["output_class"]: sum(candidate["output_class"] == row["output_class"] for candidate in rows)
        for row in rows
    } == {
        "cellular_functional_readout": 145,
        "cleavage_or_processing_readout": 20,
        "gene_expression_or_transcription": 111,
        "generic_functional_readout": 50,
        "phagocytosis_or_engulfment": 20,
        "phosphorylation_or_activation_readout": 123,
        "reporter_readout": 38,
        "secreted_protein_or_release": 51,
        "target_gene_expression": 6,
        "translocation_or_localization_readout": 30,
    }
    ...
    for row in rows:
        ...
        assert row["source_evidence_record_id"]
        assert row["primary_locator"]
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 1799
    assert len(all_queue_ids - covered_after) == 1532
    assert len(p1_queue_ids - covered_after) == 500

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 17876
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 1829
    assert {row["source_chain_id"] for row in route_rows[-594:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_fifty_second_primary_literature_expansion_batch_closes_remaining_p1_queue_with_explicit_uncertainty():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch152.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion151"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion152"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 500
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(1830, 2330)
    }
    assert sum(row["route_tier"] == "ligand_receptor_output_annotation_missing_intracellular_and_tf" for row in rows) == 236
    assert sum(row["route_tier"] == "ligand_receptor_downstream_claim_unresolved" for row in rows) == 257
    assert sum(row["route_tier"] == "receptor_intracellular_output_annotation_missing_ligand_receptor_and_tf" for row in rows) == 6
    assert sum(row["route_tier"] == "ligand_receptor_entry_only" for row in rows) == 1
    assert all(row["causal_status"] == "not_asserted" for row in rows)
    assert all(row["traversal_status"] == "evidence_route_not_causal" for row in rows)
    assert all(row["curation_status"] == "curated_primary_route" for row in rows)
    assert all(not row["transcription_factor_node_id"] for row in rows)
    assert all(not row["target_gene_node_id"] for row in rows)
    assert all(not row["output_node_id"] for row in rows)
    assert sum(row["output_class"] == "unknown" for row in rows) == 1
    ...
    for row in rows:
        if row["route_tier"] == "receptor_intracellular_output_annotation_missing_ligand_receptor_and_tf":
            assert not row["ligand_node_id"]
            assert not row["ligand_receptor_edge_id"]
            assert edges[row["receptor_intracellular_edge_id"]] == (
                row["receptor_node_id"],
                row["intracellular_continuation_node_id"],
            )
    ...
    p1_queue_ids = {row["queue_id"] for row in queue_rows if row["curation_priority"] == "P1"}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 2299
    assert len(all_queue_ids - covered_after) == 1032
    assert len(p1_queue_ids - covered_after) == 0

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 18376
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 2329
    assert {row["source_chain_id"] for row in route_rows[-500:]} == {row["expansion_id"] for row in rows}


def test_one_hundred_fifty_third_primary_literature_expansion_batch_closes_remaining_queue_with_output_uncertainty_preserved():
    root = Path(__file__).parents[1]
    input_path = root / "work/module_b_consolidation/module21b/module21b_literature_expansion_batch153.json"
    source_bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion152"
    bundle = root / "data/processed/mechanism_graph_module20_24_v2026_09_16_literature_expansion153"
    rows = read_input(input_path)
    validate_rows(rows, source_bundle)
    assert len(rows) == 1032
    assert {row["expansion_id"] for row in rows} == {
        f"M21B-LITEXP-{number:04d}" for number in range(2330, 3362)
    }
    assert sum(row["route_tier"] == "ligand_receptor_output_annotation_missing_intracellular_and_tf" for row in rows) == 26
    assert sum(row["route_tier"] == "ligand_receptor_downstream_claim_unresolved" for row in rows) == 154
    assert sum(row["route_tier"] == "ligand_receptor_entry_only" for row in rows) == 769
    assert sum(row["route_tier"] == "receptor_intracellular_claim_unresolved" for row in rows) == 78
    assert sum(row["route_tier"] == "receptor_intracellular_output_annotation_missing_ligand_receptor_and_tf" for row in rows) == 5
    assert sum(row["output_class"] != "unknown" for row in rows) == 45
    assert sum(row["output_class"] == "unknown" for row in rows) == 987
    assert all(row["causal_status"] == "not_asserted" for row in rows)
    assert all(row["traversal_status"] == "evidence_route_not_causal" for row in rows)
    assert all(row["curation_status"] == "curated_primary_route" for row in rows)
    assert all(not row["transcription_factor_node_id"] for row in rows)
    ...
    all_queue_ids = {row["queue_id"] for row in queue_rows}
    covered_after = prior_queue_ids | current_queue_ids
    assert len(covered_after) == 3331
    assert len(all_queue_ids - covered_after) == 0
    assert not {
        row["queue_id"] for row in queue_rows if row["curation_priority"] in {"P1", "P2", "P3"}
    } - covered_after

    route_rows = list(csv.DictReader(
        (bundle / "mechanism_signaling_route_evidence.tsv").open(), delimiter="\t"
    ))
    assert len(route_rows) == 19408
    assert sum(row["route_evidence_id"].startswith("LITEXP:") for row in route_rows) == 3361
    assert {row["source_chain_id"] for row in route_rows[-1032:]} == {row["expansion_id"] for row in rows}
