#!/usr/bin/env python3
"""Promote one validated Module 21A surface-enzyme relay row."""
...
REVIEW_FILES = [
    RELAY / "module21a_pair_relay_review_batches096_097.tsv",
    RELAY / "module21a_pair_relay_review_batches098_099.tsv",
]
AUDIT = RELAY / "module21a_relay_promotion_batch080.tsv"
SUMMARY = RELAY / "module21a_relay_promotion_batch080_summary.json"

PACKET = [
    ("M21A-PAIR-EVID-2286", "M20A-EXT-0823", "M21A-REUSE-1395"),
    ("M21A-PAIR-EVID-2307", "M20A-EXT-0859", "M21A-REUSE-1398"),
]

PROMOTION_NOTE = (
    "Module 21A qualified-relay promotion batch080 (2026-09-02): exact pair interaction "
    "and bounded receptor-like or receptor-proximal relay/function are raised to high at "
    "the validated layer. Surface-enzyme, intracellular-domain, complex topology, ligand "
    "versus cofactor, species/model, assay, pathway, and no-terminal-TF/SCI boundaries "
    "remain explicit; no unsupported canonical receptor or ligand claim is made."
)
...
def main():
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        if (
            row is None
            or row.get("confidence_tier") not in {"medium-high", "moderate-high"}
            or row.get("pathway_reuse_key") != reuse_key
            or not any(
                marker in row.get("evidence_layer", "")
                for marker in ("receptor_proximal_relay", "receptor-proximal_relay")
            )
        ):
        ...
        if (
            ...
            or review.get("confidence_tier") not in {"medium-high", "moderate-high"}
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        audit_rows.append({
            ...
            "decision_basis": (
                "Exact pair interaction and qualified bounded relay/function support "
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        reuse[reuse_key]["validation_status"] = "promoted_high_batch080"
    ...
    SUMMARY.write_text(json.dumps({
        "promotion_id": "module21a-surface-enzyme-batch080-2026-09-02",
