#!/usr/bin/env python3
"""Promote five validated Module 21A MIA/MMP/MSTN relay rows."""
...
REVIEW_FILES = [
    RELAY / "module21a_pair_relay_review_batches140_141.tsv",
    RELAY / "module21a_pair_relay_review_batches142_143.tsv",
]
PROMOTION_GLOB = "module21a_relay_promotion_batch*.tsv"
AUDIT = RELAY / "module21a_relay_promotion_batch091.tsv"
SUMMARY = RELAY / "module21a_relay_promotion_batch091_summary.json"

PACKET = [
    ("M21A-PAIR-EVID-3384", "M20A-EXT-2471", "M21A-REUSE-1700"),
    ("M21A-PAIR-EVID-3396", "M20A-EXT-2485", "M21A-REUSE-1705"),
    ("M21A-PAIR-EVID-3403", "M20A-EXT-2492", "M21A-REUSE-1710"),
    ("M21A-PAIR-EVID-3406", "M20A-EXT-2499", "M21A-REUSE-1711"),
    ("M21A-PAIR-EVID-3408", "M20A-EXT-2501", "M21A-REUSE-1713"),
]

PROMOTION_NOTE = (
    "Module 21A qualified-relay promotion batch091 (2026-09-02): exact pair-associated "
    "receptor-proximal relay is raised to high at the validated layer. MIA/beta1-integrin "
    "heterodimer, MMP7/SDC1 shedding and integrin-output, MMP9/Neu1/NMBR/TLR9 platform, "
    "MSTN type-II/type-I receptor redundancy, processing/source-form, complex, species/model, "
    "assay, pathway, and no-direct-binding boundaries where recorded remain explicit. "
    "Existing Module 22A terminal-TF or pending-handoff metadata is preserved unchanged; "
...
def main():
    ...
    allowed_layers = {"receptor_proximal_relay"}
    allowed_module22a_status = {"no_terminal_tf_evidence", "candidate_tf_handoff_pending_validation"}
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        if (
            row is None
            or row.get("confidence_tier", "").strip() != "medium"
        ...
        if (
            ...
            or review.get("confidence_tier", "").strip() != "medium"
        ...
        if (
            ...
            or pair.get("module22a_status") not in allowed_module22a_status
        ):
        ...
        audit_rows.append({
            ...
            "decision_basis": (
                "Exact receptor-proximal relay supports qualified-high promotion; recorded "
                "topology, context, and Module 22A TF metadata remain unchanged."
            ),
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        reuse[reuse_key]["validation_status"] = "promoted_high_batch091"
    ...
    SUMMARY.write_text(json.dumps({
        "promotion_id": "module21a-mia-mmp-mstn-relay-batch091-2026-09-02",
