#!/usr/bin/env python3
"""Integrate the second bounded high-confidence non-direct Module 20B packet."""
...
A_EVIDENCE = STARTER / "module20a_non_direct_high_batch002_evidence_register.tsv"
A_DECISIONS = STARTER / "module20a_non_direct_high_batch002_decision_register.tsv"
A_SUMMARY = STARTER / "module20a_non_direct_high_batch002_summary.json"
B_AUDIT = B_DIR / "module20b_non_direct_high_batch002_2026_09_01.tsv"
B_EVIDENCE = B_DIR / "module20b_evidence_register.tsv"

EDGE_TO_PRIMARY = {
    "M20B-E001874": "M20B-EVID-005263-P2-RELAY",
    "M20B-E001881": "M20B-EVID-005244-P2-RELAY",
    "M20B-E001885": "M20B-EVID-005245-P2-RELAY",
    "M20B-E001901": "M20B-EVID-005274-P2-RELAY",
    "M20B-E002537": "M20B-EVID-002537-P2-FUNC",
    "M20B-E002887": "M20B-EVID-005295-P2-RELAY",
    "M20B-E004404": "M20B-EVID-004404-P2-RELAY",
    "M20B-E004422": "M20B-EVID-004422-P2-RELAY",
    "M20B-E004423": "M20B-EVID-004423-P2-RELAY",
    "M20B-E004442": "M20B-EVID-004442-P2-RELAY",
    "M20B-E004443": "M20B-EVID-004443-P2-RELAY",
}
...
BASIS = (
    "Exact pair-specific primary receptor-proximal relay or downstream/function evidence was adjudicated high in Module 20B. High applies only to the preserved non-direct evidence layer; it does not establish binary binding, a terminal TF, SCI relevance, or an outcome beyond the cited record. Species, complex, isoform, ligand-form, assay, and model limitations remain explicit."
)
...
def main() -> None:
    ...
    if any(frozen[e]["confidence_tier"] not in {"medium", "high"} for e in EDGE_TO_PRIMARY):
        raise SystemExit("selected frozen LR evidence has an unexpected confidence tier")
    for edge_id, evidence_id in EDGE_TO_PRIMARY.items():
        ...
        if p["evidence_layer"] not in {"receptor_proximal_relay", "receptor_proximal_or_pathway", "downstream_or_functional", "downstream_pathway_or_cellular_function"} or p["exportable"].casefold() != "true":
            raise SystemExit(f"primary evidence is not exportable non-direct evidence for {edge_id}")
    ...
    for index, edge_id in enumerate(sorted(EDGE_TO_PRIMARY), start=1):
        ...
        fr = frozen[edge_id]
        new_id = f"M20A-NONDIRECTHIGH002-EVID-{index:04d}"
        ...
        q["curator_notes"] = append_once(q["curator_notes"], "Non-direct-high batch002: exact primary relay/function evidence adjudicated high for the preserved non-direct layer; no binary binding, TF, downstream, or SCI inference added.")
        edge.update({"confidence_tier": "high", "export_priority": "high", "exportable": "true", "consolidation_note": append_once(edge["consolidation_note"], "Non-direct-high batch002: exact primary relay/function evidence promoted at the non-direct layer; directness and context boundaries retained.")})
