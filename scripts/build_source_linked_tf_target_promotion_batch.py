#!/usr/bin/env python3
"""Promote missing TF and relay layers from exact target-route templates.

This pass handles source-linked target-gene routes with the shape
``ligand>receptor>????>????>target_gene_expression``.  It only uses a full
route with the same ligand, receptor, and target-gene node identities as a
template for the missing intracellular and TF identities.  The source-linked
partial row remains the provenance anchor; no graph edge, causal assertion, or
confidence score is created.
"""
...
def make_row(
    ...
    lr_edge_id = matching_edge(
        partial.get("ligand_receptor_edge_id", ""),
        edges,
        ligand_node_id,
        receptor_node_id,
    ) or matching_edge(
        template.get("ligand_receptor_edge_id", ""),
        edges,
        ligand_node_id,
        receptor_node_id,
    )
    ...
    linkage = join_unique(
        ...
        "literature_expansion:source_linked_tf_target_promotion",
        "exact_ligand_receptor_target_template",
        "relay_and_tf_identities_supported_by_existing_full_route",
    ...
    summary = source_record.get("evidence_summary", "").strip() or (
        f"Source-linked route component for {partial['ligand_label']} > {partial['receptor_label']} > "
        f"???? > ???? > {partial['target_gene_label']}."
    )
    limitations = join_unique(
        source_record.get("limitations", ""),
        template_record.get("limitations", ""),
        "The intracellular continuation and TF identities are composed from an existing full route with the same ligand, receptor, and target-gene node identities; the partial route's source did not itself assert those layers.",
    ...
    row.update(
        {
            "expansion_id": f"M21B-LITEXP-TFPROMO-{ordinal:05d}",
            ...
            "source_chain_id": f"tf_target_promotion:{partial['expansion_id']}:{template['expansion_id']}",
            "route_linkage_status": linkage,
            "primary_locator": join_unique(partial.get("primary_locator", ""), template.get("primary_locator", "")),
            "citation_note": (
                f"The target-gene route is retained from {partial['expansion_id']}; the relay and TF identities "
                f"are composed from {template['expansion_id']} with the same ligand, receptor, and target-gene "
                "node identities. No new graph edge or causal handoff is asserted."
            ),
...
def make_rows(bundle: Path) -> tuple[list[dict[str, str]], dict[str, int]]:
    ...
    templates_by_key: dict[tuple[str, str, str], list[dict[str, str]]] = defaultdict(list)
    for row in expansions:
        if (
            ...
            templates_by_key[
                (row["ligand_node_id"], row["receptor_node_id"], row["target_gene_node_id"])
            ].append(row)

    rows: list[dict[str, str]] = []
    generated_signatures: set[tuple[str, ...]] = set()
    skipped = {"no_exact_template": 0, "duplicate_signature": 0, "missing_primary_locator": 0, "missing_source_linkage": 0}
    ...
    for partial in partials:
        ...
        if not partial.get("source_queue_id") or not partial.get("source_evidence_record_id"):
            ...
            continue
        key = (partial.get("ligand_node_id", ""), partial.get("receptor_node_id", ""), partial.get("target_gene_node_id", ""))
        templates = templates_by_key.get(key, [])
        if not templates:
            skipped["no_exact_template"] += 1
            continue
        for template in templates:
            signature = (
                ...
                template.get("transcription_factor_node_id", ""),
...
def main() -> int:
    ...
    if not rows:
        raise ValueError("No source-linked TF-target promotions found")
