#!/usr/bin/env python3
"""Validate CellChat non-LR audit reports."""
...
BATCH = ROOT / "work" / "module20_db_seed" / "evidence_search_agents" / "cellchat_nonlr_audit_batch_001"
...
ALLOWED = {"retain_non_ligand_receptor", "restore_low", "restore_medium", "retain_manual"}
...
def main() -> None:
    ...
    for item in read(BATCH / "manifest.tsv"):
        expected.extend(row["pair_key"] for row in read(ROOT / item["packet_path"]))
        ...
        for row in rows:
            ...
            if row["final_class"] in {"restore_low", "restore_medium"} and not row["source_locators"].strip():
                errors.append(f"restoration without source locator {row['pair_key']}")
        reports.extend(rows)
