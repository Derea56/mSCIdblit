#!/usr/bin/env python3
"""Promote validated glutamate/glycine receptor rows from the 1561–1610 audit."""
...
REVIEW_FILES = [RELAY / "module21a_pair_relay_review_batches068_069.tsv"]
AUDIT = RELAY / "module21a_relay_promotion_batch070.tsv"
SUMMARY = RELAY / "module21a_relay_promotion_batch070_summary.json"

PACKET = [(
    f"M21A-PAIR-EVID-{evidence_number}",
    f"M20A-CELLCHAT-REMAINING-{evidence_number - 441:04d}",
    f"M21A-REUSE-{evidence_number - 477:04d}",
) for evidence_number in range(1561, 1584)]
assert len(PACKET) == 23

PROMOTION_NOTE = (
    "Module 21A receptor promotion batch070 (2026-09-02): exact mature ligand activation "
    "of the specified receptor complex is raised to high at the validated receptor-proximal "
    "relay layer. Source-processing labels remain context only; exact receptor composition, "
    "model, species, assembly, and assay boundaries remain explicit, with no terminal-TF or "
    "SCI claim."
)
...
def main():
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        audit_rows.append({
            ...
            "decision_basis": (
                "Exact mature-ligand receptor activation supports qualified-high promotion at "
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        reuse[reuse_key]["validation_status"] = "promoted_high_batch070"
    ...
    SUMMARY.write_text(json.dumps({
        "promotion_id": "module21a-receptor-batch070-2026-09-02",
