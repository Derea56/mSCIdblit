#!/usr/bin/env python3
"""Integrate priority-6 low ECM batches 018-019 from agent TSV audits."""
...
REVIEW = RELAY / "module21a_pair_relay_review_batches018_019.tsv"
...
def main() -> None:
    ...
    source_rows = []
    for path in (RELAY / "module21a_batch018_review.tsv", RELAY / "module21a_batch019_review.tsv"):
        for r in read(path):
            source_rows.append({"review_id": r["review_id"], "source_locators": r["stable_citations"],
                                ...
                                "confidence_tier": "low", "terminal_tf_entities": "" if r.get("terminal_TF", "") in {"", "—"} else r["terminal_TF"],
                                "review_status": r["status"], "curator_note": r["limitations"]})
    if len(source_rows) != 50 or len({r["review_id"] for r in source_rows}) != 50:
        raise SystemExit("batches018_019 must contain 50 unique exact rows")
    ...
    for s in source_rows:
        ...
        pair["search_boundary"] = "primary_review_batches018_019; exact family/chain boundary search"
        ...
        tfrow["search_boundary"] = "Primary downstream review batches018-019; exact family/chain boundary search."
        ...
        if has_reuse:
            new_reuse.append({"pathway_reuse_key": reuse_key, "source_entity": s["pathway_branch"].split("-", 1)[0],
                              ...
                              "validation_status": "reviewed_batches018_019_candidate",
