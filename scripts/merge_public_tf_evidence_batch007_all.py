#!/usr/bin/env python3
"""Merge the bounded Batch 007 evidence lanes into one audit register."""
...
def main() -> int:
    ...
    pair_paths = [
        input_dir / "public_tf_evidence_review_batch007_ranks0105_pairs.tsv",
        input_dir / "public_tf_evidence_review_batch007_ranks0625_pairs.tsv",
    ]
    register_paths = [
        input_dir / "public_tf_evidence_review_batch007_ranks0105_regulator_register.tsv",
        input_dir / "public_tf_evidence_review_batch007_ranks0625_regulator_register.tsv",
    ]
    ...
    if {int(row["batch_rank"]) for row in pairs} != set(range(1, 26)):
        raise SystemExit("unexpected batch ranks")
    if any(row.get("exportable", "").lower() != "false" or row.get("canonical_write_performed", "").lower() != "false" for row in pairs + evidence):
        raise SystemExit("promotion/write guard failed")

    pair_path = input_dir / "public_tf_evidence_review_batch007_pairs.tsv"
    register_path = input_dir / "public_tf_evidence_review_batch007_regulator_register.tsv"
    summary_path = input_dir / "batch_007_evidence_review_summary.json"
    report_path = input_dir / "EVIDENCE_REVIEW_BATCH007.md"
    ...
    for row in evidence:
        support_kind_counts[row["support_kind"]] = support_kind_counts.get(row["support_kind"], 0) + 1
    summary = {
        "status": "staging_only", "batch_id": "public_tf_evidence_batch_007", "source_set_ranks_reviewed": list(range(1, 26)),
        "pair_rows_reviewed": len(pairs), "unique_regulators_reviewed": len({row["regulator_key"] for row in pairs}),
        "unique_provenance_aware_pairs": len(pair_keys), "exact_pair_status_counts": exact_status_counts,
        "regulator_register_rows": len(evidence), "regulator_support_kind_counts": support_kind_counts,
        "exact_pair_support_rows": 0, "partial_or_near_match_rows": 0,
        "binding_dataset_only_not_promotable_rows": sum(row["exact_pair_status"] == "binding_dataset_only_not_promotable" for row in pairs),
        "exportable_pair_rows": 0, "canonical_writes_performed": False, "module22b_promotions_performed": False,
        "provenance_preserved": True,
        "limitations": [
            "No-exact-support is bounded to the PubMed/PMC and authoritative dataset searches recorded in each lane; it is not a global absence claim.",
    ...
    report_path.write_text(
        "# Evidence review: public TF batch 007\n\n"
        ...
        "The binding-only records are useful search leads and preserve TFLink provenance, but they do not establish target-cell presence, effect direction, downstream activation, or an upstream relay.\n\n"
