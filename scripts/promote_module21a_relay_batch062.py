#!/usr/bin/env python3
"""Promote independently verified mature-GABA/GABAA complex packets."""
...
REVIEW_FILES = [
    RELAY / "module21a_pair_relay_review_batches048_049.tsv",
    RELAY / "module21a_pair_relay_review_batches050_051.tsv",
]
AUDIT = RELAY / "module21a_relay_promotion_batch062.tsv"
SUMMARY = RELAY / "module21a_relay_promotion_batch062_summary.json"

# The eight GABAB/unresolved rows in this range are intentionally excluded.
SAFE_EVID = [
    evid for evid in range(1061, 1111)
    if evid not in {1075, 1076, 1085, 1092, 1093, 1102, 1109, 1110}
]
PACKET = [
    (
        f"M21A-PAIR-EVID-{evid}",
        f"M20A-CELLCHAT-REMAINING-{evid - 458:04d}",
        f"M21A-REUSE-{642 + index:04d}",
    )
    for index, evid in enumerate(SAFE_EVID)
]

PROMOTION_NOTE = (
    "Module 21A receptor-complex promotion batch062 (2026-09-02): mature GABA activation "
    "of the exact recombinant GABAA subunit complex is raised to high at the receptor-proximal "
    "channel/function layer. GAD2 and SLC32A/SLC6A synthesis or vesicular transport remain "
    "separate source context; native abundance, stoichiometry, localization, ionic conditions, "
    "and model-specific outputs remain bounded, with no terminal-TF or SCI claim."
)
...
def main():
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        if (
            ...
            or not review.get("pair_key", "").startswith("gaba-gad")
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        reuse[reuse_key]["validation_status"] = "promoted_high_batch062"
    ...
    SUMMARY.write_text(
        json.dumps({
            "promotion_id": "module21a-receptor-complex-batch062-2026-09-02",
            "records_promoted": len(audit_rows),
            "evidence_ids": [r["evidence_id"] for r in audit_rows],
            "promotion_note": PROMOTION_NOTE,
            "excluded_evidence_ids": [f"M21A-PAIR-EVID-{evid}" for evid in (1075, 1076, 1085, 1092, 1093, 1102, 1109, 1110)],
