#!/usr/bin/env python3
"""Add a direct retinoid-receptor target lane recovered from remaining holds."""
...
AUDIT_PATH = ROOT / "work/module22b_low_confidence_upgrade_audit/module22b_narrative_tf_targets_batch016.tsv"
SUMMARY_PATH = ROOT / "work/module22b_low_confidence_upgrade_audit/module22b_narrative_tf_targets_batch016_summary.json"
BATCH_ID = "module22b-narrative-tf-targets-batch016-2026-09-03"

UPDATES = [
    (
        "M22B-HOLD-AUDIT-0049;M22B-HOLD-AUDIT-0050;M22B-HOLD-AUDIT-0051;M22B-HOLD-AUDIT-0052;M22B-HOLD-AUDIT-0053",
        "RARA;RXRA",
        "SALL4A",
        "mouse",
        "PMID:26427057",
        "Mouse spermatogonial differentiation model with RAR/RXR loss-of-function, conserved regulatory-region analysis, and ATRA-dependent SALL4A induction; non-SCI comparator.",
        "The primary study showed that ATRA activates RAR/RXR heterodimers bound at a conserved SALL4A regulatory region and increases SALL4A transcription in spermatogonia.",
        "This is a RARA/RXRA-complex target-gene edge from a germ-cell differentiation model; it does not resolve the submitted receptor-specific ALDH1A3 branch or establish SCI specificity.",
    ),
]
...
def main() -> int:
    ...
    audit = []
    for index, (holds, tf, target, species, locator, context, summary, limitations) in enumerate(UPDATES, start=108):
