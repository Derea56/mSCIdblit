...
AUDIT = RELAY / "module21a_relay_promotion_batch061.tsv"
SUMMARY = RELAY / "module21a_relay_promotion_batch061_summary.json"

# The six GABAB1/GABAB2 subunit rows in this range are intentionally excluded.
SAFE_EVID = [
    evid for evid in range(1011, 1061)
    if evid not in {1024, 1025, 1041, 1042, 1058, 1059}
]
PACKET = [
    (
        f"M21A-PAIR-EVID-{evid}",
        f"M20A-CELLCHAT-REMAINING-{evid - 458:04d}",
        f"M21A-REUSE-{598 + index:04d}",
    )
    for index, evid in enumerate(SAFE_EVID)
]

PROMOTION_NOTE = (
    "Module 21A receptor-complex promotion batch061 (2026-09-02): mature GABA activation "
    "of the exact recombinant GABAA subunit complex is raised to high at the receptor-proximal "
    "channel/function layer. GAD1/GAD2 and SLC6A synthesis or vesicular transport remain separate "
    "source context; native abundance, stoichiometry, localization, ionic conditions, and model-"
    "specific outputs remain bounded, with no terminal-TF or SCI claim."
)
...
def main():
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        reuse[reuse_key]["validation_status"] = "promoted_high_batch061"
    ...
    SUMMARY.write_text(
        json.dumps({
            "promotion_id": "module21a-receptor-complex-batch061-2026-09-02",
            "records_promoted": len(audit_rows),
            "evidence_ids": [r["evidence_id"] for r in audit_rows],
            "promotion_note": PROMOTION_NOTE,
            "excluded_gabab_subunit_evidence_ids": [f"M21A-PAIR-EVID-{evid}" for evid in (1024, 1025, 1041, 1042, 1058, 1059)],
