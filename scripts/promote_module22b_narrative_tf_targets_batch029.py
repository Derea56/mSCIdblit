#!/usr/bin/env python3
"""Add an EGR1 target recovered while reviewing Module 22B hold 0023."""
...
AUDIT_PATH = ROOT / "work/module22b_low_confidence_upgrade_audit/module22b_narrative_tf_targets_batch029.tsv"
SUMMARY_PATH = ROOT / "work/module22b_low_confidence_upgrade_audit/module22b_narrative_tf_targets_batch029_summary.json"
BATCH_ID = "module22b-narrative-tf-targets-batch029-2026-09-03"

UPDATES = [
    (
        "M22B-HOLD-AUDIT-0023",
        "EGR1",
        "SOX9",
        "mouse",
        "PMID:35910788; PMCID:PMC9330523",
        "Primary mouse renal tubular-cell ischemia/reperfusion and folic-acid acute-kidney-injury models with EGR1 ChIP and Sox9 promoter reporter/perturbation experiments; injury-repair non-SCI comparator.",
        "EGR1 directly bound the Sox9 promoter and increased Sox9 expression in renal tubular epithelial cells; Egr1 loss reduced the repair-associated SOX9 response.",
        "This is a standalone EGR1 target-gene edge from an acute-kidney-injury repair model; it does not establish the submitted FGF23-FGFR1-to-EGR1 handoff or SCI specificity.",
    ),
]
...
def main() -> int:
    ...
    audit = []
    for index, (holds, tf, target, species, locator, context, summary, limitations) in enumerate(UPDATES, start=135):
