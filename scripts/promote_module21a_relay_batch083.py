#!/usr/bin/env python3
"""Promote three validated Module 21A bounded relay/function rows."""
...
REVIEW_FILES = [RELAY / "module21a_pair_relay_review_batches106_107.tsv"]
PROMOTION_GLOB = "module21a_relay_promotion_batch*.tsv"
AUDIT = RELAY / "module21a_relay_promotion_batch083.tsv"
SUMMARY = RELAY / "module21a_relay_promotion_batch083_summary.json"

PACKET = [
    ("M21A-PAIR-EVID-2532", "M20A-EXT-1106", "M21A-REUSE-1445"),
    ("M21A-PAIR-EVID-2533", "M20A-EXT-1107", "M21A-REUSE-1446"),
    ("M21A-PAIR-EVID-2535", "M20A-EXT-1109", "M21A-REUSE-1448"),
]

PROMOTION_NOTE = (
    "Module 21A qualified-relay promotion batch083 (2026-09-02): exact pair-associated "
    "function or receptor-proximal relay is raised to high at the validated layer. CD14 "
    "cofactor/accessory, TLR1:TLR2 or TLR6-containing complex, Fas/TRIF/RIPK1 death-complex, "
    "ligand-form, species/model, assay, pathway-versus-terminal-TF, and no-SCI boundaries "
    "remain explicit; no unsupported standalone CD14 binding or universal complex claim is made."
)
...
def main():
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        if (
            ...
            or not any(
                ...
                for marker in ("receptor_proximal_relay", "downstream_pathway_function")
            )
        ):
        ...
        if (
            ...
            or review.get("review_status") not in {"reviewed_function_only", "reviewed_relay_candidate"}
        ):
        ...
        audit_rows.append({
            ...
            "decision_basis": (
                "Exact pair-associated function or receptor-proximal relay supports "
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        reuse[reuse_key]["validation_status"] = "promoted_high_batch083"
    ...
    SUMMARY.write_text(json.dumps({
        "promotion_id": "module21a-bounded-relay-batch083-2026-09-02",
