#!/usr/bin/env python3
"""Promote validated glutamate-receptor rows from the 1361–1410 audit."""
...
AUDIT = RELAY / "module21a_relay_promotion_batch065.tsv"
SUMMARY = RELAY / "module21a_relay_promotion_batch065_summary.json"

PACKET = []
for evidence_number in [*range(1361, 1384), *range(1386, 1409)]:
    review_number = evidence_number - 441
    reuse_number = evidence_number - (461 if evidence_number < 1386 else 463)
    PACKET.append((
        f"M21A-PAIR-EVID-{evidence_number}",
        f"M20A-CELLCHAT-REMAINING-{review_number:04d}",
        f"M21A-REUSE-{reuse_number:04d}",
    ))
assert len(PACKET) == 46

PROMOTION_NOTE = (
    "Module 21A glutamate-receptor promotion batch065 (2026-09-02): exact mature glutamate "
...
def main():
    ...
    for evidence_id, review_id, reuse_key in PACKET:
        ...
        reuse[reuse_key]["validation_status"] = "promoted_high_batch065"
    ...
    SUMMARY.write_text(json.dumps({
        "promotion_id": "module21a-glutamate-receptor-batch065-2026-09-02",
