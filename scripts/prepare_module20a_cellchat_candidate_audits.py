#!/usr/bin/env python3
"""Prepare citation-level audits for CellChat promote candidates."""
...
BATCH = WORK / "evidence_search_agents" / "cellchat_candidate_audit_001"
...
def main() -> None:
    ...
    if not candidates:
        raise SystemExit("no CellChat promote candidates found")
    sizes = [60] * (len(candidates) // 60)
    if len(candidates) % 60:
        sizes.append(len(candidates) % 60)
    manifest = []
    offset = 0
    for index, size in enumerate(sizes, 1):
        packet_id = f"M20A-CELLCHAT-AUDIT-{index:02d}"
    ...
    (BATCH / "worker_contract.md").write_text(
        """# CellChat candidate citation-audit contract
...
""",
    ...
    print(f"prepared {len(candidates)} CellChat candidate audits across {len(sizes)} packets")
