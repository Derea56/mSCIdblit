#!/usr/bin/env python3
"""Merge the bounded Batch 005 evidence lanes into one audit register."""
...
def main() -> int:
    ...
    pair_paths = [
        input_dir / "public_tf_evidence_review_batch005_ranks0105_pairs.tsv",
        input_dir / "public_tf_evidence_review_batch005_ranks0625_pairs.tsv",
    ]
    register_paths = [
        input_dir / "public_tf_evidence_review_batch005_ranks0105_regulator_register.tsv",
        input_dir / "public_tf_evidence_review_batch005_ranks0625_regulator_register.tsv",
    ]
    ...
    pair_path = input_dir / "public_tf_evidence_review_batch005_pairs.tsv"
    register_path = input_dir / "public_tf_evidence_review_batch005_regulator_register.tsv"
    summary_path = input_dir / "batch_005_evidence_review_summary.json"
    report_path = input_dir / "EVIDENCE_REVIEW_BATCH005.md"
    ...
    summary = {
        "status": "staging_only", "batch_id": "public_tf_evidence_batch_005",
    ...
    report_path.write_text(
        "# Evidence review: public TF batch 005\n\n"
        ...
        f"- **{summary['partial_or_near_match_rows']}** pairs were partial or binding-only leads.\n"
        f"- **{len(evidence)}** regulator-level or authoritative binding records were retained separately.\n"
        "- **0** rows are exportable; no canonical or Module 22B writes were performed.\n\n"
        "Regulator-level records are evidence that a candidate has experimental regulatory capacity or a relevant authoritative occupancy resource. They are not evidence for any queued target edge and were kept separate to prevent cross-module contamination.\n\n"
