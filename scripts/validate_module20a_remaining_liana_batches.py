#!/usr/bin/env python3
"""Validate remaining LIANA search reports without mutating canonical data."""
...
BATCH = ROOT / "work" / "module20_db_seed" / "evidence_search_agents" / "liana_remaining_batch_001"
...
def main() -> None:
    ...
    summary = {"generated_utc": datetime.now(UTC).isoformat(timespec="seconds"), "expected_rows": len(expected_ids), "reported_rows": len(reports), "verdict_counts": dict(sorted(Counter(row["verdict"] for row in reports).items())), "validation_errors": errors, "canonical_ledgers_mutated": False}
