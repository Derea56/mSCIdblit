...
The 12 exact source-artifact identity decisions are now materialized as an
...
main identity resolver only when the cited local XML contains exactly one
metadata-bearing paper record, the PMID is explicit in that record, and the
same PMID is already present in the original composite key.
...
identity manifest increased from 3,705 to 3,717 rows with a resolved PMID;
the unresolved total decreased from 1,017 to 1,005, and the ambiguous
multiple-canonical-PMID count decreased from 499 to 487. The original
...
The next queue pass should focus on the remaining 89 identity-resolution
