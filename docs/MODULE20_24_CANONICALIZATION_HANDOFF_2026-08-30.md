...
## Exact duplicate queue adjudication — 2026-09-01

The first queue audit applied a conservative exact-reuse rule to all 426
...
lane resolved 16 units where one source-record artifact contained one PMID
already present in the original composite key and no competing PMID was
selected. It created no duplicate Paper, Observation,
...
The reproducible audit is `scripts/adjudicate_module20_24_review_queue.py`.
Its outputs are:
...
The remaining 387 units stay pending manual source review or authoritative
...
## Queue-artifact identity resolver checkpoint — 2026-09-01

The 16 exact source-artifact identity decisions are now materialized as an
explicit resolver input at
`work/cross_module_synthesis/canonical_evidence_review/module20_24_phase2_paper_identity_queue_artifact_resolutions.tsv`.
The ledger is regenerated by
`scripts/adjudicate_module20_24_review_queue.py` and contains the exact cited
source locator, row-level extraction ID, original composite paper key,
resolved PMID/PMCID/DOI, and parsed source metadata. It is accepted by the
main identity resolver only when the cited local XML or HTML contains exactly
one metadata-bearing paper record, the PMID is explicit in that record, and the
same PMID is already present in the original composite key. The checker now
uses the format-aware XML/HTML source-record parser for those formats while
retaining keyed-identifier restrictions for JSON artifacts.

The resolver now recognizes Europe PMC core XML responses as source records
when their XML elements contain the PMID and title. On rerun, the Phase-2
identity manifest increased from 3,705 to 3,721 rows with a resolved PMID;
the unresolved total decreased from 1,017 to 1,001, and the ambiguous
multiple-canonical-PMID count decreased from 499 to 483. The original
composite keys, evidence grades, context levels, claims, and database schema
were unchanged. The downstream paper/evidence materializers were rerun; they
remain generated SQL only and no canonical database write was performed.

The next queue pass should focus on the remaining 85 identity-resolution
units, then the 141 exact-identity/source-review units. Composite keys with
